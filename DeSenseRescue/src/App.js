...
class App extends Component {
    ...
    getAllCauses = () => {
        ...
        console.log(DeSenseRescueInstance);
        DeSenseRescueInstance.methods.getAllCauses().call().then((response) => {
    ...
    addCause = () => {
        if (this.state.goal<=0)
            {
                alert('Cause goal should be more than 0 ETH');
                return;
            }
        else if (this.state.title && this.state.type && this.state.description && this.state.goal) {
            this.toggleBlocking();
            DeSenseRescueInstance.methods.startCause(
                this.state.title,
                this.state.type,
            ...
            alert('Please fill all the fields to Create a New Cause');
            return;
        }
    }
    ...
    donate = () => {
        if (this.state.currentCause) {
            ...
            left = left.toFixed(1)
            //console.log("GOAL:",Number(toEther(this.state.currentCause.c_goal)),"RAISED:",Number(toEther(this.state.currentCause.c_raised)),"LEFT:",left,"RAISED BY ME:",this.state.raised)
        ...
        return (
            <React.Fragment>
                <BlockUi blocking={this.state.blocking}>
                    <section className="section causes" style={appStyle} >
                        <div className="container" >
                            <div className="row margin-bottom" >
                                <div className="col-lg-4" >
                                    <div className="heading heading--primary"><span className="heading__pre-title color--green" >What we Do</span>
                                        <h2 className="heading__title no-margin-bottom"><span>DeSense Rescue</span> <span>Causes</span></h2>
                                    </div>
                                </div>
                                <div className="col-lg-8">
                                    <p className="causes__heading-text">
                                        We connect communities in crisis with donors worldwide. Our decentralized platform ensures transparency, security, and efficiency in disaster relief donations.
                                        <br /><br />
                                        Join us in making a difference—<strong>support a cause today!</strong>
                                    </p>
                                </div>
                            </div>
                            <div className={'text-center mb-2'}>
                                <button data-toggle={'modal'} data-target={'#cretecausemodal'}
                                    style={{ background: '#3b1b00', color: 'white', fontSize:'20px'}} className="button causes-item__button button--secondary d-none d-sm-inline-block">
                                    Create new Cause
                                </button>
                            </div>
                            <div className="row">
                                {
                                    this.state.causes.map((item, index) => {
                                        //console.log(item.c_raised, item.c_goal);
                                        return (
                                            <div key={index} className="col-lg-6">
                                                <div className="causes-item causes-item--style-2">
                                                    <div className="causes-item__body">
                                                        {
                                                            (item.c_starter === web3.eth.defaultAccount || item.c_raised >= item.c_goal) ? (
                                                                <span style={{ color: '#ccffcc' }}>✔ Completed</span>
                                                              ) : (
                                                                <div className="causes-item__action">
                                                                  ...
                                                                  <a
                                                                    ...
                                                                    data-toggle="modal" data-target={'#donateModal'} href="#">+ Donate
                                                                  </a>
                                                                </div>
                                                              )
                                                        }
                    ...
                    <div id="cretecausemodal" className="modal fade" role="dialog" style={{fontFamily:"'Montserrat',serif",fontWeight:'bold', fontSize:'20px'}}>
                        <div className="modal-dialog" >
                            <div className="modal-content" style={{ background: '#fff0e0', color:'black'}}>
                                <div className="modal-header">
                                    <h4 className="modal-title">Create a New Cause</h4>
                                ...
                                <div className="modal-footer text-center">
                                    <button
                                        ...
                                        type="button" style={{ background: '#3b1b00', color: 'white' }}
                    ...
                    <div id="donateModal" className="modal fade" role="dialog" style={{fontFamily:"'Montserrat',serif",fontWeight:'bold', fontSize:'20px'}}>
                        <div className="modal-dialog">
                            <div className="modal-content" style={{ background: '#fff0e0'}}>
                                <div className="modal-header">
                                ...
                                <div className="modal-footer text-center">
                                    <button
                                        onClick={this.donate}
                                        type="button" style={{ background: '#634e35', color: 'white' }}
                                        className="button causes-item__button button--secondary d-none d-sm-inline-block" >Save
                                    </button>
                                </div>
                            </div>
